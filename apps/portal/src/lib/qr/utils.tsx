...
export function getImageSettings(
  ...
  const x = imageSettings.x == null ? qrCodeSize / 2 - w / 2 : imageSettings.x * scale;
  const y = imageSettings.y == null ? qrCodeSize / 2 - h / 2 : imageSettings.y * scale;
...
export function QRCodeSVG(props: QRPropsSVG) {
  ...
  const shouldUseHigherErrorLevel = isOGContext && imageSettings?.excavate && (level === "L" || level === "M");
  ...
  let cells = qrcodegen.QrCode.encodeText(value, ERROR_LEVEL_MAP[effectiveLevel] as qrcodegen.QrCode.Ecc).getModules();
  ...
  if (imageSettings != null && calculatedImageSettings != null) {
    ...
    if (isOGContext) {
      const { imgWidth, imgHeight, imgLeft, imgTop } = convertImageSettingsToPixels(
        calculatedImageSettings,
        size,
        numCells,
        margin
  ...
  return (
    <svg height={size} viewBox={`0 0 ${numCells} ${numCells}`} width={size} {...otherProps}>
