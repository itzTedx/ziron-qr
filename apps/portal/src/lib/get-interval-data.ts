...
import { ZIRON_FUNDING_DATE } from "./constants";
...
> = {
  ...
  all: () => ({
    startDate: ZIRON_FUNDING_DATE,
    granularity: "month",
  }),
};

export function getIntervalData(interval: string, { timezone }: { timezone?: string } = {}) {
  const intervalFn = INTERVAL_DATA[interval];
  if (!intervalFn) return undefined;
  return intervalFn({ timezone });
}
