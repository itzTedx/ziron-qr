...
export function UploadDropzoneProgress({
	...
	useEffect(() => {
		progresses.forEach((progress) => {
			const previousProgress = previousProgresses.current.find(
				(p) => p.objectInfo.key === progress.objectInfo.key
			);
			...
			if (progress.progress < 1 && progress.status !== "failed") {
				toast.custom(
					() => (
						<div
							className={cn(
								"relative flex w-[356px] items-center gap-2 rounded-lg border bg-accent p-3",
								{
