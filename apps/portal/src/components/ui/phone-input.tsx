          ...
          <CommandList className="max-h-60 overflow-auto">
            {countries.map((country) => (
              <CommandItem
                ...
                value={en[country]}
              >
                <FlagComponent aria-hidden="true" country={country} countryName={en[country]} />
                <span className="text-sm">{en[country]}</span>
                <span className="ml-auto text-muted-foreground text-xs">+{getCountryCallingCode(country)}</span>
              </CommandItem>
            ))}
          </CommandList>
        </Command>
