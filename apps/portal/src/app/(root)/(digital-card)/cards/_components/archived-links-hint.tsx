...
function ArchivedLinksHintHelper({ setShowArchived }: { setShowArchived: (showArchived: boolean) => void }) {
	const { data: nonArchivedCount } = useSuspenseQuery(
		orpc.card.count.queryOptions({ input: { showArchived: false } })
	);
				...
				<TooltipContent>
					<div className="flex items-center gap-4">
						<span>
							...
							{pluralize("link", archivedCount)} that match{archivedCount === 1 && "es"}
							the applied filters
						</span>
