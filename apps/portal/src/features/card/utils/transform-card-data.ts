...
export function transformCardData(initialData?: Partial<CardType>): zCardSchema {
  return {
    ...
    bio: initialData?.bio ?? "",
    appearance: {
      template: initialData?.appearance?.template ?? "default",
      theme: initialData?.appearance?.theme ?? "#4938ff",
      btnColor: initialData?.appearance?.btnColor ?? "#4938ff",
      isDarkMode: initialData?.appearance?.isDarkMode ?? false,
    },
