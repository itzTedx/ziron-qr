...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      className={cn(
        "flex flex-col gap-0 rounded-lg border bg-card py-1.5 text-card-foreground shadow-sm [corner-shape:squircle] md:rounded-2xl",
        className
      )}
      data-slot="card"
      {...props}
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div className={cn("flex items-center px-1.5 [.border-t]:pt-1.5", className)} data-slot="card-footer" {...props} />
  );
}
