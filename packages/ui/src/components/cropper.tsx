...
import { cn, useComposedRefs } from "@ziron/utils";
    ...
    setState: (key, value) => {
      ...
      if (key === "crop" && typeof value === "object" && value && "x" in value) {
        ...
        if (value) {
          ...
          if (currentState?.mediaSize && currentState.cropSize && onCropComplete) {
            const { croppedAreaPercentages, croppedAreaPixels } = getCroppedArea(
              currentState.crop,
  ...
  return (
    <StoreContext.Provider value={store}>
      <CropperContext.Provider value={contextValue}>
        <div className={cn("relative size-full overflow-hidden", className)} data-slot="cropper-wrapper">
...
function CropperRootImpl(props: CropperRootImplProps) {
  ...
  const onWheelZoom = React.useCallback(
    ...
    [contentProps.onWheelZoom, getMousePoint, zoom, context.zoomSpeed, onZoomChange, getWheelDelta, store]
  );
  ...
  const onMouseDown = React.useCallback(
    ...
    [contentProps.onMouseDown, getMousePoint, onDragStart, onDragStopped, onMouseMove, onContentPositionChange]
  );
  ...
  const onTouchStart = React.useCallback(
    ...
    [contentProps.onTouchStart, onDragStopped, onTouchMove, onContentPositionChange, getTouchPoint, onDragStart]
  );
...
function CropperVideo(props: CropperVideoProps) {
  const { className, style, asChild, ref, onLoadedMetadata, objectFit, snapPixels = false, ...videoProps } = props;
...
function CropperArea(props: CropperAreaProps) {
  const { className, style, asChild, ref, snapPixels = false, shape, withGrid, ...areaProps } = props;
