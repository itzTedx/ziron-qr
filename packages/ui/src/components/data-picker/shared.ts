    ...
    presets.forEach((preset) => {
      if ("date" in preset) {
        ...
        if (startMonthToUse) {
          if (presetMonth < startMonthToUse.getMonth()) {
            throw new Error(`Preset ${preset.label} is before startMonth ${format(startMonthToUse, "MMM, yyyy")}.`);
          }
        }

        if (endMonthToUse) {
          if (presetMonth > endMonthToUse.getMonth()) {
            throw new Error(`Preset ${preset.label} is after endMonth ${format(endMonthToUse, "MMM, yyyy")}.`);
          }
        }

        if (startDateToUse) {
          if (preset.date.getTime() < startDateToUse.getTime()) {
            throw new Error(`Preset ${preset.label} is before startDate ${format(startDateToUse, "MMM dd, yyyy")}.`);
          }
        }

        if (endDateToUse) {
          if (preset.date.getTime() > endDateToUse.getTime()) {
            throw new Error(`Preset ${preset.label} is after endDate ${format(endDateToUse, "MMM dd, yyyy")}.`);
          }
        }
      }
      ...
      if ("dateRange" in preset) {
        ...
        if (endMonthToUse && preset.dateRange.to) {
          ...
          if (presetMonth > endMonthToUse.getMonth()) {
            throw new Error(`Preset ${preset.label}'s 'to' is after endMonth ${format(endMonthToUse, "MMM, yyyy")}.`);
          }
        }
        ...
        if (endDateToUse && preset.dateRange.to) {
          ...
          if (presetToTime > endDateToUse.getTime()) {
            throw new Error(`Preset ${preset.label}'s 'to' is after endDate ${format(endDateToUse, "MMM dd, yyyy")}.`);
          }
        }
      }
    });
